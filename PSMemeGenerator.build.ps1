...
<#
.SYNOPSIS
    Build script for the 'PSMemeGenerator' PowerShell module

.DESCRIPTION
    This script contains the tasks for building the 'PSMemeGenerator' PowerShell module
#>
[CmdletBinding()]
...
Enter-Build {
    $script:moduleName = 'PSMemeGenerator'
...
task Export-CommandHelp {
    ...
    $helpFiles = Measure-PlatyPSMarkdown -Path "$buildPath/help/PSMemeGenerator/*.md"
    foreach ($helpFile in $helpFiles) {
        [void] (Test-MarkdownCommandHelp -Path $helpFile.FilePath)
    }

    # Generate module help file
    $mdfiles = Measure-PlatyPSMarkdown -Path "$buildPath/help/PSMemeGenerator/*.md" | Where-Object Filetype -Match 'CommandHelp'
